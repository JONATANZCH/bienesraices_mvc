...
const registrar = async (req, res) => { 
    ...
    await check('password').isLength({min: 6}).withMessage('El Password debe tener mínimo 6 caracteres').run(req)
    ...
    //? Verificar que no esté duplicado el usuario
...
const confirmar = async (req, res) => { 
    ...
    if(!usuario){
        return res.render('auth/confirmar-cuenta', {
            ...
            mensaje: 'Hubo un error al confirmar tu cuenta, vuelve a intentarlo',
            error: true
        })
    }
...
const comprobarToken = async (req, res) => { 
    ...
    if(!usuario) {
        return res.render('auth/confirmar-cuenta', {
            pagina: 'Rastablece tu Password',
            mensaje: 'Hubo un error al validar tu información, vuelve a intentarlo',
            error: true
        })
    }
